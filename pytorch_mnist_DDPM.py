...
bs = 100
results_folder = Path("./samples/ddpm/mnist")
...
def train(model, optimizer, epoch, train_hist):
    ...
    # if (np.argmax(train_hist['losses']) == len(train_hist['losses']) - 1):
    #     torch.save(model, 'models/ddpm_mnist.pth')
...
if __name__ == '__main__':
    ...
    for i in range(5):
        ax = axs[0, i]
        t = 60 * i + 59
        img = samples[t][random_index].reshape(image_size, image_size, channels).cpu()
