...
bs = 100
results_folder = Path("./samples/ddpm")
...
test_dataset = datasets.MNIST(
    root=data_path_root, train=False, transform=transform, download=False)
    ...
    for step, (batch, _) in enumerate(train_loader):
        ...
        for i in tqdm(reversed(range(0, timesteps)), desc='sampling loop time step', total=timesteps):
            ...
            if i == 0:
                ...
                posterior_variance_t = extract(posterior_variance, t, shape)
                noise = torch.randn_like(img)
                # Algorithm 2 line 4:
                img = model_mean + torch.sqrt(posterior_variance_t) * noise
        D_output = D(img).squeeze()
...
if __name__ == '__main__':
    ...
    # torch.save(model, 'models/ddpm.pth')

    # sample 1 images
    samples = sample(model, image_size=image_size,
                     batch_size=64, channels=channels)

    random_index = torch.randint(0, 64, (1,))
    fig, axs = plt.subplots(figsize=(200, 200), nrows=1,
                            ncols=5, squeeze=False)
    for i in range(5):
        ax = axs[0, i]
        t = 60 * i + 59
        img = samples[t][random_index].reshape(image_size, image_size, channels)
        ax.imshow(np.asarray(img), cmap='gray')
        ax.set(xticklabels=[], yticklabels=[], xticks=[], yticks=[])
    plt.tight_layout()
    plt.savefig(results_folder/'sample_step.png')

    fig = plt.figure()
    ims = []
    for i in range(timesteps):
        im = plt.imshow(samples[i][random_index].reshape(
            image_size, image_size, channels).cpu(), cmap="gray", animated=True)
        ims.append([im])
    animate = animation.ArtistAnimation(
        fig, ims, interval=50, blit=True, repeat_delay=5000)
    animate.save(results_folder / 'sample_step.gif')